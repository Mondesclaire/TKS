...
**Формат числа:** $Q_{I.F}$ или $Q_{m.n}$, где:
...
$2^{I-1}, 2^{I-2}, \dots, 2^1, 2^0, 2^{-1}, 2^{-2}, \dots, 2^{-F}$
...
- Веса дробной части: $2^{-1}, 2^{-2}, \dots, 2^{-7}$
...
$$ \sigma_q = \frac{\Delta}{\sqrt{12}} \approx 0.2887 \cdot \Delta $$

**Отношение сигнал/шум (SNR) для синусоидального сигнала с амплитудой A, оцифрованного с разрядностью N бит (без знака):**

Мощность сигнала: $P_s = \frac{A^2}{2}$

Максимальная амплитуда без клиппинга: $A_{max} = 2^{N-1} \cdot \Delta$ (для знакового формата)

$$ P_{s,max} = \frac{(2^{N-1} \cdot \Delta)^2}{2} = 2^{2N-3} \cdot \Delta^2 $$

$$ SNR = \frac{P_{s,max}}{\sigma_q^2} = \frac{2^{2N-3} \cdot \Delta^2}{\Delta^2 / 12} = \frac{3}{2} \cdot 2^{2N} $$
...
$$ Q(x) = \Delta \cdot \lfloor \frac{x}{\Delta} + 0.5 \rfloor $$

**Ошибка:** $e \in [-\Delta/2, \Delta/2]$, равномерное распределение.
...
**Ошибка:** для положительных x ошибка $\in [0, \Delta)$, для отрицательных x ошибка $\in (-\Delta, 0]$.

#### 1.3.4 Округление с избытком (Round toward +∞, Ceil)

$$ Q(x) = \Delta \cdot \lceil \frac{x}{\Delta} \rceil $$

Всегда округление вверх. Ошибка $e \in [0, \Delta)$ для всех x.

#### 1.3.5 Округление к нулю (Round toward 0)

$$ Q(x) = \Delta \cdot \text{sign}(x) \cdot \lfloor \frac{|x|}{\Delta} \rfloor $$
...
| :--- | :--- | :--- | :--- |
| К ближайшему (Half-Up) | $[-\Delta/2, \Delta/2]$ | 0 | Стандарт IEEE 754 |
| К ближайшему четному | $[-\Delta/2, \Delta/2]$ | 0 | Финансы, БПФ |
| Усечение (к нулю) | $[0, \Delta)$ для $x \ge 0$; $(-\Delta, 0]$ для $x < 0$ | $\approx -\Delta/2$ | Цифровые фильтры |
| С избытком (Ceil) | $[0, \Delta)$ | $+\Delta/2$ | Специальные алгоритмы |
| К нулю (симметричное) | $(-\Delta, \Delta)$ | 0 | DSP-процессоры |
...
**Определение:** Отрицательное число -x в N-битном представлении получается как $2^N - x$, что эквивалентно инверсии всех битов с последующим прибавлением 1.
...
$$ S(t) = \text{Re}\{ (I(t) + jQ(t)) e^{j\omega_c t} \} = I(t) \cos(\omega_c t) - Q(t) \sin(\omega_c t) $$
...
$$ S_{BPSK}(t) = A \cdot m(t) \cos(\omega_c t), \text{ где } m(t) = +1 \text{ или } -1 $$
...
- 00 → фаза 45°: $I = +1/\sqrt{2}, Q = +1/\sqrt{2}$
- 01 → фаза 135°: $I = -1/\sqrt{2}, Q = +1/\sqrt{2}$
- 11 → фаза 225°: $I = -1/\sqrt{2}, Q = -1/\sqrt{2}$
- 10 → фаза 315°: $I = +1/\sqrt{2}, Q = -1/\sqrt{2}$
...
**Формула символа:** $C = I + jQ = Ae^{j\phi}$
...
**Сетка:** 8×8 = 64 точки.
**Координаты:** $I, Q \in \{-7, -5, -3, -1, +1, +3, +5, +7\}$.
...
| **Мин. расстояние ($d_{min}$)** | 2.0 | 1.414 | 0.765 | ~0.632 | ~0.308 | ~0.154 |
| **Требуемый $E_b/N_0$** | ~9.5 дБ | ~9.5 дБ | ~13.5 дБ | ~14.5 дБ | ~18.5 дБ | ~23.5 дБ |
...
$$ EVM = \frac{\sqrt{\frac{1}{N} \sum |E_k|^2}}{P_{ref}} \cdot 100\% $$

где:
- $E_k$ — вектор ошибки для k-го символа
- $P_{ref}$ — средняя мощность идеального созвездия

**Связь с SNR:**

$$ SNR \approx \frac{1}{EVM^2} \text{ (в линейных единицах)} $$
