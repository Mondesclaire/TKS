...
**Формат числа:** Q_I.F или Qm.n, где:
...
2<sup>(I-1)<sup>, 2^(I-2), ..., 2^1, 2^0, 2^(-1), 2^(-2), ..., 2^(-F)
...
- Вычисление: 0·8 + 1·4 + 1·2 + 0·1 + 1·0.5 + 0·0.25 + 1·0.125 + 0·0.0625 = 4 + 2 + 0.5 + 0.125 = 6.625₁₀
...
- Диапазон: от -1 до +1 - 2^(-7) ≈ +0.9921875
...
σ_q² = ∫ e²·(1/Δ) de = Δ²/12  [интеграл от -Δ/2 до Δ/2]
...
Мощность сигнала: P_s = A²/2

Максимальная амплитуда без клиппинга: A_max = 2^(N-1)·Δ (для знакового формата)

P_s,max = (2^(N-1)·Δ)²/2 = 2^(2N-3)·Δ²

SNR = P_s,max / σ_q² = (2^(2N-3)·Δ²) / (Δ²/12) = (3/2)·2^(2N)

**В децибелах:**

SNR_dB = 10·log₁₀((3/2)·2^(2N)) = 10·log₁₀(1.5) + 20N·log₁₀(2) ≈ 1.76 + 6.02N дБ
...
где A(t) — амплитуда, φ(t) — фаза, ω_c — несущая частота.

Переход к комплексной огибающей:

S(t) = Re{ (I(t) + j·Q(t))·e^(j·ω_c·t) } = I(t)·cos(ω_c·t) - Q(t)·sin(ω_c·t)
...
- Умножение на e^(jφ) соответствует повороту фазы
...
S_BPSK(t) = A·m(t)·cos(ω_c·t), где m(t) = +1 или -1.
...
**Формула символа:** C = I + j·Q = A·e^(j·φ)
...
EVM = [ √( (1/N)·Σ|E_k|² ) / P_ref ] · 100%

где:
- E_k — вектор ошибки для k-го символа
- P_ref — средняя мощность идеального созвездия

**Связь с SNR:**

SNR ≈ 1 / EVM² (в линейных единицах)
